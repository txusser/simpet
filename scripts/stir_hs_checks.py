...
def check_scanner_radius(
    cfg: Mapping[str, Any], stir_hs: List[str], cfg_name: str, stir_hs_name: str
) -> None:
    """
    ...
    stir_diameter = [
        param.strip("\n")
        for param in stir_hs
        if re.match("Inner ring diameter \(cm\)", param)
    ]
    ...
    print(
        f"Scanner diameter extracted from 2 * scanner_radius -> 2 * {cfg_radius} in config file"
    )
    if not math.isclose(cfg_diameter, stir_diameter):
        print(
            f"Scanner diameter differs, STIR: {stir_diameter}, CFG: {cfg_diameter}.\n"
        )
    else:
        print(
            f"Scanner diameter approx. eq., STIR: {stir_diameter}, CFG: {cfg_diameter}.\n"
        )


def check_s_total_size(
    cfg: Mapping[str, Any], stir_hs: List[str], cfg_name: str, stir_hs_name: str
) -> None:
    """
    Checks if STIR's diameter is the same
    ...
    stir_bin_size = [
        param.strip("\n")
        for param in stir_hs
        if re.match("Default bin size \(cm\)", param)
    ]
    ...
    print(f"STIR HS: {stir_hs_name}, CFG: {cfg_name}.")
    print(
        f"Config scanner_radius = {scanner_radius}, config diameter = {2 * scanner_radius}."
    )
    print(f"Config num_td_bins = {num_td}.")
    print(
        f"Config bin size -> 2 * scanner_radius / num_td = {2 * scanner_radius / num_td}."
    )
    print(f"s n bins extracted from !matrix size [1] ({stir_s_bins}) in .hs file")
    print(
        f"s bin size extracted from Default bin size (cm) ({stir_bin_size}) in .hs file"
    )
