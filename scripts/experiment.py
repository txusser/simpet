...
def get_central_slices(path: Union[str, Path]) -> OrderedDict[str, np.ndarray]:
    ...
    image_files = [
        (file_.name, file_)
        for file_ in path_.rglob("**/*")
        if "".join(file_.suffixes) in image_extensions
    ]
    images = [
        (filename, nib.load(img_file).get_fdata()) for filename, img_file in image_files
    ]
...
def get_table(slices: Mapping[str, np.ndarray]) -> wandb.Table:
    ...
    table = wandb.Table(
        columns=list(plots.keys()), data=[[plot for plot in plots.values()]]
    )
...
def get_logs(path: Union[str, Path]) -> List[Path]:
    ...
    return [
        logfile
        for logfile in Path(path).rglob("**/*")
        if logfile.suffix in [ext.value for ext in LogFileExtension]
    ]
...
def copy_recon(cfg: Mapping[str, Any], path: Union[str, Path]) -> None:
    ...
    Args:
        ...
        path: path to save the config.
    """
    patient_path = Path(cfg["dir_results_path"]).joinpath(
        cfg["params"]["patient_dirname"]
    )

    try:
        recon_path = [
            p for p in patient_path.rglob("**/rec_*.img") if p.name.startswith("rec_")
        ].pop()
...
def compute_F_factor(cfg: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    cfg["params"]["scanner"]["F"] = (
        xy_output_size * transaxial_crystal_size / (2 * scanner_radius * zoom_factor)
    )
...
@hydra.main(
    version_base=None,
    config_path=str(here().joinpath("configs")),
...
def simulate(cfg: DictConfig) -> None:
    ...
    if log:
        run = wandb.init(
            project="SimPET-Randfigs-Simulations",
            config=cfg,
            name=cfg["params"]["scanner"]["scanner_name"],
            job_type=job_type,
        )
    ...
    recon_name = [
        p.parents[1].name
        for p in patient_dir.rglob("**/rec_*.img")
        if p.name.startswith("rec_")
    ].pop()
