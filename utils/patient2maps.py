...
class patient2maps(object):
    ...
    def mri_normalization(self):
        ...
        norm_mri, mri_transformation_matrix = spm.new_normalize(self.spm_run, mfile, self.mri_img, template_image,
                                                                self.log_file)
        ...
        spm.new_deformations(self.spm_run, mfile, mri_transformation_matrix, self.mri_img, images_to_deform,
                             self.analysis_path, 0, self.log_file)
    ...
    def atlas_generation(self):
        ...
        for i in [output_wm_hdr, output_csf_hdr, output_soft_hdr]:  # High priority tissues.
            ...
            tools.osrun(rcommand, self.log_file)
            rcommand = '%s -C -1 %s %s fl multi' % (opera_imagen, join(self.analysis_path, i[0:-4] + "_mask.hdr"),
                                                    join(self.analysis_path, i[0:-4] + "_mask.hdr"))
            tools.osrun(rcommand, self.log_file)

        # Generates final GM using WM and CSF masks
        rcommand = '%s %s %s %s fl multi' % (
        opera_imagen, output_gm_hdr, join(self.analysis_path, output_wm_hdr[0:-4] + "_mask.hdr"), output_gm_hdr)
        tools.osrun(rcommand, self.log_file)
        rcommand = '%s %s %s %s fl multi' % (
        opera_imagen, output_gm_hdr, join(self.analysis_path, output_csf_hdr[0:-4] + "_mask.hdr"), output_gm_hdr)
        tools.osrun(rcommand, self.log_file)

        # Generates final bone tissue using CSF and soft tissue mask
        rcommand = '%s %s %s %s fl multi' % (
        opera_imagen, output_bone_hdr, join(self.analysis_path, output_csf_hdr[0:-4] + "_mask.hdr"), output_bone_hdr)
        tools.osrun(rcommand, self.log_file)
        rcommand = '%s %s %s %s fl multi' % (
        opera_imagen, output_bone_hdr, join(self.analysis_path, output_soft_hdr[0:-4] + "_mask.hdr"), output_bone_hdr)
        tools.osrun(rcommand, self.log_file)
        ...
        if self.ct_img:  # ct_img is not empty
            rcommand = '%s %s %s %s fl multi' % (
            opera_imagen, self.ct_img[0:-3] + "hdr", pet_mask, self.ct_img[0:-3] + "hdr")
            tools.osrun(rcommand, self.log_file)
            ...
            rcommand = '%s %s %s %s fl multi' % (
            opera_imagen, output_bone_hdr[0:-4] + "_attindex.hdr", pet_mask, output_bone_hdr[0:-4] + "_attindex.hdr")
            tools.osrun(rcommand, self.log_file)
            ...
            for k in [output_gm_hdr, output_wm_hdr, output_csf_hdr, output_soft_hdr]:
                ...
                rcommand = '%s %s %s %s fl sumar' % (opera_imagen, k[0:-4] + "_attindex.hdr",
                                                     output_bone_hdr[0:-4] + "_attindex.hdr",
                                                     output_bone_hdr[0:-4] + "_attindex.hdr")
                tools.osrun(rcommand, self.log_file)

            rcommand = '%s %s %s 0.136 10 %s' % (
            cambia_val_interval, output_bone_hdr[0:-4] + "_attindex.hdr", output_bone_hdr[0:-4] + "_attindex.hdr",
            0.096)
            tools.osrun(rcommand, self.log_file)
    ...
    def cleanup(self):
        ...
        os.system("rm %s/c1* %s/c2* %s/c3* %s/c4* %s/c5*" % (
        self.analysis_path, self.analysis_path, self.analysis_path, self.analysis_path, self.analysis_path))
