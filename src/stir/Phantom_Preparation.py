...
class Phantom_Preparation(object):
    def __init__(
        self, spm_run, act_map, att_map, scanner, scanner_target_size, pet_image=False
    ):
    ...
    def phantom_preparation(self):
        ...
        zpix, zsize, xpix, xsize, ypix, ysize = apple.read_analyze_header(
            self.emiss_hdr, self.logfile
        )
        ...
        for i in [self.emiss_hdr, self.att_hdr, self.original_pet]:
            if i:
                ...
                rcommand = "cambia_matriz_imagen_hdr %s %s %s %s %s novecino" % (
                    i,
                    output_i,
                    new_x_dims,
                    new_y_dims,
                ...
                rcommand = "gen_hdr %s %s %s %s fl %s %s %s 0" % (
                    output_i[0:-4],
                    new_x_dims,
                    new_y_dims,
                    new_z_dims,
                ...
                if i == self.emiss_hdr:
                    ...
                    smoothed = spm.smoothing_xyz(
                        self.spm_run, mfile, output_i, 2, 2, 2, "s", self.logfile
                    )
                ...
                if i == self.att_hdr:
                    ...
                    rcommand = (
                        "cambia_valores_de_un_intervalo %s %s 1 10000000000000 1 "
                        % (output_i, output_i)
                    )
                    apple.osrun(rcommand, self.logfile)
