...
def copy_analyze(image1, image2=False, dest_dir=False, logfile=False):
    ...
    if image2:
        if (
            ...
            message = "Error! The provided image is not in Analyze format:" + str(
                image1
            )
            if logfile:
                log_message(logfile, message, "error")
        ...
        message = (
            "Error! Not image2 path or dest dir path provided: "
            + str(image2)
            + ", "
            + str(dest_dir)
        )
...
def create_analyze_from_imgdata(
    data, out, pix_x, pix_y, pix_z, tx, ty, tz, data_type="fl"
):
    if data_type == "1b":
        dtype = np.int8
    elif data_type == "2b":
...
def nii_analyze_convert(image, logfile=False, outfile=False):
    ...
    data = data.astype(
        np.float32
    )  # casting to avoid problems when applying nib.save with no float data
...
def petmr2maps(
    pet_image, mri_image, ct_image, log_file, spm_run, output_dir, mode="SimSET"
):
    """
    ...
    my_map_generation = patient2maps(
        spm_run, output_dir, log_file, mri_img, correg_pet_img, correg_ct_img, mode=mode
    )
...
def convert_simset_sino_to_stir(input_img, output=False):
    ...
    stir_img = nib.AnalyzeImage(
        stir_img_data_flip_x, simset_img.affine, simset_img.header
    )
...
def scalImage(image_hdr, maxValue, log_file):
    ...
    osrun(rcommand, log_file)
    rcommand = "%s %s %s 0.9 10000000000 1 >> %s" % (
        cambia_val_interval,
        mask_hdr,
        mask_hdr,
        log_file,
...
def update_act_map(
    spmrun, act_map, att_map, orig_pet, simu_pet, output_act_map, axialFOV, log_file
):
    ...
    operate_single_image(
        coreg_simpet_hdr, "mult", norm_factor, coreg_simpet_hdr, log_file
    )
    ...
    s_coreg_simpet_img = spm.smoothing(
        spmrun, mfileSmooth, coreg_simpet_img, 5, "s", log_file
    )
    ...
    change_interval_values(
        division_hdr, division_hdr, 5, 100000000000000000000000000000000000, 1
    )
...
def fsl_flirt(reference_hdr, input_hdr, log_file):
    ...
    flt.cmdline
    "flirt -in %s -ref %s -out %s -dof 6 -cost mutualinfo -searchrx -30 30 -searchry -30 30 -searchrz -30 30" % (
        input_hdr,
        reference_hdr,
        coreg_hdr,
    )
