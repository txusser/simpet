...
def write_interfile_header(header_file, matrix_size_x, pixel_size_x, matrix_size_y, pixel_size_y, matrix_size_z,
                           pixel_size_z, offset_z=0):
...
def anything_to_hdr_convert(image, logfile=False, outfile=False):
    ...
    if image[-3:] == "hdr" or image[-3:] == "img":
        ...
        create_analyze_from_imgdata(data_file, hdr_header, float(pixel_x), float(pixel_y), float(pixel_z),
                                    float(pixel_size_x), float(pixel_size_y), float(pixel_size_z))
...
def makeImageSquare(image_hdr, log_file):
    ...
    if pixDif != 0:
        ...
        if xpix > ypix:
            ...
            corta_pega_filcol_hdr, image_hdr, image_hdr, "1", str(oneSide), log_file)
            rcommand2 = '%s %s %s peg %s %s >> %s' % (
            corta_pega_filcol_hdr, image_hdr, image_hdr, "2", str(otherSide), log_file)
        elif ypix > xpix:
            rcommand1 = '%s %s %s peg %s %s >> %s' % (
            corta_pega_filcol_hdr, image_hdr, image_hdr, "3", str(oneSide), log_file)
            rcommand2 = '%s %s %s peg %s %s >> %s' % (
            corta_pega_filcol_hdr, image_hdr, image_hdr, "4", str(otherSide), log_file)
...
def fsl_flirt(reference_hdr, input_hdr, log_file):
    ...
    input_hdr, reference_hdr, coreg_hdr)
    aux = os.getcwd()
