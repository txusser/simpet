...
class STIR_Simulation(object):
    ...
    def __init__(self, scanner, simpet_dir, simulation_name, results_name, act_map, att_map, pet_image=False):
        # Configuring inputs
    ...
    def run(self):
        ...
        """
        print
        "Applying Positron range and non-colinearity smoothing"
    ...
    def positron_range_non_col_filter(self, input_image):
        ...
        smoothed_img = tools.smoothing(self.matlab_rcommand, mfile_name, img_file, self.prnc_fwhm, "s", self.logfile)
        ...
        def create_line_integrals(self):
            ...
            rcommand = '%s/forward_project %s/my_line_integrals.hs %s %s %s' % (
                self.stir_dir, self.results_dir, self.emiss_map, self.scanner_template, self.proyector)
        ...
        def calculate_attenuation_coefficients(self):
            ...
            Uses forward_project to project the activity sinograms!
            """
            rcommand = '%s/calculate_attenuation_coefficients --PMRT --ACF %s/my_acfs.hs %s %s' % (
        ...
        def apply_normalization_factors(self):
            ...
            rcommand = '%s/stir_math -s --including-first --times-scalar 0 --add-scalar 3.4 %s/my_norm.hs %s/my_acfs.hs' % (
                self.stir_dir, self.results_dir, self.results_dir)
            tools.osrun(rcommand, self.logfile)

            rcommand = '%s/stir_math -s -mult %s/my_multifactors.hs %s/my_norm.hs %s/my_acfs.hs' % (
        ...
        def create_random_coincidences(self):
            ...
            rcommand = '%s/stir_math -s --including-first --times-scalar 0 --add-scalar 100 %s/my_randoms.hs %s/my_line_integrals.hs' % (
                self.stir_dir, self.results_dir, self.results_dir)
            tools.osrun(rcommand, self.logfile)
            ...
            rcommand = '%s/stir_divide -s --accumulate --times-scalar 0 --add-scalar 100 %s/my_randoms.hs %s/my_line_integrals.hs' % (
                self.stir_dir, self.results_dir, self.results_dir)

            $DIR_STIR / stir_divide - s - -accumulate $DIR_PROJECTIONS / my_randoms.hs $DIR_PROJECTIONS / my_norm.hs >> $DIR_LOGS / stir_math.log
            2 >> $DIR_LOGS / stir_math.errors
