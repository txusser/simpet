...
def apply_psf(scannerParams, sinogram_stir, log_file):
    ...
    conv_sino2proy, sinogram_stir[0:-3] + 's', num_aa_bins, num_td_bins, cortes, projections[0:-3] + 'img')
    ...
    command = "%s %s fl %s %s %s %s fl" % (
    conv_proy2sino, conv_projections[0:-3] + 'img', num_aa_bins, num_td_bins, cortes, sinogram_stir[0:-3] + 's')
    tools.osrun(command, log_file)
...
def add_noise(config, scannerParams, sinogram_stir, log_file):
    ...
    command = "%s -p %s %s %s %s" % (
    poison_noise, noisy_sinogram_stir_path, sinogram_stir[0:-3] + 'hs', scannerParams.get('add_noise'),
...
def OSEM2D_recons(config, scannerParams, sinograms_stir, additive_sino_stir, att_stir, output_dir, log_file):
    ...
    if scannerParams.get("inter_iteration_filter") == 1:  # will apply inter-iteration filter
        inter_iter_filter_str = (
                "inter-iteration filter subiteration interval:= " + str(
            scannerParams.get("subiteration_interval")) + "\n" +
    ...
    new_file.write(
        ...
        "zoom := " + str(zoom) + "\n" +
        "xy output image size (in pixels) := " + str(xyOutputSize) + "\n"
                                                                     "Z output image size (in pixels) := " + str(
            zOutputSize) + "\n\n" +
