...
class SimSET_Simulation(object):
    ...
    def __init__(self, params, config, act_map, att_map, scanner, projections_dir,
                 debug=False):  ##Debug True must be implemented....
    ...
    def prepare_simset_files(self, sim_dir, act_table_factor, act, sim_photons, sim_time, sampling):
        ...
        simset_tools.make_simset_phg(self.config, my_phg_file, sim_dir, act, scanner_radius, scanner_axial_fov,
                                     self.center_slice,
                                     isotope, sim_photons, sim_time, add_randoms, self.phglistmode, sampling,
                                     log_file=log_file)
    ...
    def run_recons(self):
        ...
        if any(exists(i) == False for i in [sinogram_stir, additive_sino_stir, att_stir]):
            ...
            if recons_algorithm == 'FBP2D':
                reconsFile_hdr = stir_tools.FBP2D_recons(self.config, self.scanner, sinogram_stir, self.output_dir,
                                                         self.log_file)

            elif recons_algorithm == 'FBP3D':
                reconsFile_hdr = stir_tools.FBP3D_recons(self.config, self.scanner, sinogram_stir, self.output_dir,
                                                         self.log_file)

            elif recons_algorithm == 'OSEM2D':
                reconsFile_hdr = stir_tools.OSEM2D_recons(self.config, self.scanner, sinogram_stir, additive_sino_stir,
                                                          att_stir, self.output_dir, self.log_file)

            elif recons_algorithm == 'OSEM3D':
                reconsFile_hdr = stir_tools.OSEM3D_recons(self.config, self.scanner, sinogram_stir, additive_sino_stir,
                                                          att_stir, self.output_dir, self.log_file)
