...
class wholebody_simulation(object):
    ...
    def run(self):
        ...
        beds_cs = wb_tools.calculate_center_slices(
            act_map, self.scanner, self.zmin, self.zmax
        )
        ...
        for cs in beds_cs:
            recons_dir = join(
                output_dir,
                "Bed_cs_%s" % cs,
                "%s_Sim_%s" % (self.sim_type, self.scanner_model),
                recons_algorithm,
            )
            recons_file = join(
                recons_dir, "rec_%s_%s.hdr" % (recons_algorithm, recons_it)
            )
