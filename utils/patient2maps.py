...
class patient2maps(object):
    ...
    def mri_normalization(self):
        ...
        norm_mri, mri_transformation_matrix = spm.new_normalize(
            self.spm_run, mfile, self.mri_img, template_image, self.log_file
        )
        ...
        spm.new_deformations(
            self.spm_run,
            mfile,
            mri_transformation_matrix,
            self.mri_img,
            images_to_deform,
            self.analysis_path,
            0,
    ...
    def atlas_generation(self):
        ...
        """
        message = (
            "I will generate the ACT and ATT maps for %s simulation..." % self.mode
        )
        print(message)
        ...
        output_gm_hdr = tools.nii_analyze_convert(
            join(self.analysis_path, "c1" + mri_basename[0:-3] + "nii")
        )
        output_wm_hdr = tools.nii_analyze_convert(
            join(self.analysis_path, "c2" + mri_basename[0:-3] + "nii")
        )
        output_csf_hdr = tools.nii_analyze_convert(
            join(self.analysis_path, "c3" + mri_basename[0:-3] + "nii")
        )
        output_bone_hdr = tools.nii_analyze_convert(
            join(self.analysis_path, "c4" + mri_basename[0:-3] + "nii")
        )
        output_soft_hdr = tools.nii_analyze_convert(
            join(self.analysis_path, "c5" + mri_basename[0:-3] + "nii")
        )
        ...
        rcommand = "%s %s %s %s fl multi" % (
            opera_imagen,
            output_gm_hdr,
            join(self.analysis_path, output_wm_hdr[0:-4] + "_mask.hdr"),
            output_gm_hdr,
        )
        tools.osrun(rcommand, self.log_file)
        rcommand = "%s %s %s %s fl multi" % (
            opera_imagen,
            output_gm_hdr,
            join(self.analysis_path, output_csf_hdr[0:-4] + "_mask.hdr"),
            output_gm_hdr,
        )
        tools.osrun(rcommand, self.log_file)
        ...
        rcommand = "%s %s %s %s fl multi" % (
            opera_imagen,
            output_bone_hdr,
            join(self.analysis_path, output_soft_hdr[0:-4] + "_mask.hdr"),
            output_bone_hdr,
        )
        tools.osrun(rcommand, self.log_file)
